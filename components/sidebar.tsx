...
const Sidebar = ({ apiLimitCount }: { apiLimitCount: number }) => {
    ...
    return (
        <div className="p-4 flex flex-col h-full overflow-y-auto bg-[#111827] text-white no-scrollbar">
            ...
            <div className="mt-auto bg-white/10 flex flex-col items-center gap-2 justify-center h-max px-2 py-3 rounded-md">
                <p>
                    {`${apiLimitCount}/${MAX_FREE_TRAIL_COUNT}`} Free
                    Generations
                </p>
                <Progress
                    value={(apiLimitCount / MAX_FREE_TRAIL_COUNT) * 100}
                    className="h-3"
                />
                <Button
                    className="w-full"
